...
@BackboneRegistry.register("ncsnpp-ctm_48k")
class NCSNppCTM_48k(nn.Module):
    """NCSN++ model, adapted from https://github.com/yang-song/score_sde repository"""
    ...
    def forward(self, x, t_time_cond, s_time_cond):
        # timestep/noise_level embedding; only for continuous training
        modules = self.all_modules
        m_idx = 0
        ...
        if self.embedding_type == 'fourier':
            # Gaussian Fourier features embeddings for t
            used_sigmas = t_time_cond
            temb = modules[m_idx](torch.log(used_sigmas))
            m_idx += 1
            # Gaussian Fourier features embeddings for s
            s_used_sigmas = s_time_cond
            semb = modules[m_idx](torch.log(s_used_sigmas))
            m_idx += 1
            temb = temb + semb
        elif self.embedding_type == 'positional':
            # Sinusoidal positional embeddings.
            timesteps = t_time_cond
            used_sigmas = self.sigmas[t_time_cond.long()]
            temb = layers.get_timestep_embedding(timesteps, self.nf)

            s_timesteps = s_time_cond
            s_used_sigmas = self.sigmas[s_time_cond.long()]
            semb = layers.get_timestep_embedding(s_timesteps, self.nf)
