...
def si_sdr_loss(s, s_hat):
    alpha = (s_hat * s).sum(dim=1) / torch.norm(s, dim=1) ** 2
    num = torch.norm(alpha[:, None] * s, dim=1) ** 2
    den = torch.norm(alpha[:, None] * s - s_hat, dim=1) ** 2
    sdr = 10 * torch.log10(num / (den + 1e-8))
    sdr_loss = -sdr
    return sdr_loss
