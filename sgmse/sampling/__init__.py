...
def get_ode_sampler(
    ...
    def denoise_update_fn(x):
        ...
        if 1:
            _, x = predictor.update_fn(x, y, vec_eps, sde.N)
        else:
...
def get_sb_sampler_ode(sde, model, x_t, y, timesteps_list):
    # adapted from https://github.com/NVIDIA/NeMo/blob/78357ae99ff2cf9f179f53fbcb02c88a5a67defb/nemo/collections/audio/parts/submodules/schroedinger_bridge.py#L382
    """The SB-ODE sampler function."""
    with torch.no_grad():
        for b in range(len(timesteps_list)):
            time_steps = timesteps_list[b]
            x_t_b = x_t[b].unsqueeze(0)
            y_b = y[b].unsqueeze(0)

            if time_steps.numel() > 0:
                # Initial values
                time_prev = time_steps[0] * torch.ones(1, device=x_t_b.device)
                sigma_prev, sigma_T, sigma_bar_prev, alpha_prev, alpha_T, alpha_bar_prev = sde._sigmas_alphas(time_prev)

            for t in time_steps[1:]:
                # Prepare time steps for the whole batch
                time = t * torch.ones(1, device=x_t_b.device)

                # Get noise schedule for current time
                sigma_t, sigma_T, sigma_bart, alpha_t, alpha_T, alpha_bart = sde._sigmas_alphas(time)

                # Run DNN
                current_estimate = model(x_t_b, y_b, time)

                # Calculate scaling for the first-order discretization from the paper
                weight_prev       = alpha_t * sigma_t * sigma_bart / (alpha_prev * sigma_prev * sigma_bar_prev + sde.eps)
                weight_estimate   = alpha_t / (sigma_T ** 2 + sde.eps) * (sigma_bart ** 2 - sigma_bar_prev * sigma_t * sigma_bart / (sigma_prev + sde.eps))
                weight_prior_mean = alpha_t / (alpha_T * sigma_T ** 2 + sde.eps) * (sigma_t ** 2 - sigma_prev * sigma_t * sigma_bart / (sigma_bar_prev + sde.eps))
                ...
                # Update state: weighted sum of previous state, current estimate and prior
                x_t_b = weight_prev * x_t_b + weight_estimate * current_estimate + weight_prior_mean * y_b
                ...
                sigma_prev = sigma_t
                sigma_bar_prev = sigma_bart
            x_t[b] = x_t_b.squeeze(0)

        return x_t
    
def get_sb_sampler_ode_ctm(sde, model, x_t, y, time_steps):
    # adapted from https://github.com/NVIDIA/NeMo/blob/78357ae99ff2cf9f179f53fbcb02c88a5a67defb/nemo/collections/audio/parts/submodules/schroedinger_bridge.py#L382
    """The SB-ODE sampler function."""
    with torch.no_grad():
        # Initial values
        time_prev = time_steps[0] * torch.ones(x_t.shape[0], device=x_t.device)
        sigma_prev, sigma_T, sigma_bar_prev, alpha_prev, alpha_T, alpha_bar_prev = sde._sigmas_alphas(time_prev)

        for t in time_steps[1:]:
            # Prepare time steps for the whole batch
            time = t * torch.ones(x_t.shape[0], device=x_t.device)

            # Get noise schedule for current time
            sigma_t, sigma_T, sigma_bart, alpha_t, alpha_T, alpha_bart = sde._sigmas_alphas(time)

            # Run DNN
            current_estimate = model.g_th(x_t, time, time, y, model.dnn)
            ...
            # Update state: weighted sum of previous state, current estimate and prior
            x_t = weight_prev * x_t + weight_estimate * current_estimate + weight_prior_mean * y
...
def get_sb_sampler_sde_1step(sde, model, y):
    """The SB-SDE sampler function."""
    with torch.no_grad():
        xt = y[:, [0], :, :] # special case for storm_2ch
        time_steps = torch.linspace(sde.T, sde.T - 1e-3, 2, device=y.device)
